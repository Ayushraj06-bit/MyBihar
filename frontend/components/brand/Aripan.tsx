...
export function AripanRule({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={`hb-rule-wrap ${className}`} ref={ref} aria-hidden="true">
      <svg className={`hb-rule ${seen ? 'is-drawn' : ''}`} viewBox="0 0 600 44" preserveAspectRatio="none" fill="none">
        <path className="hb-rule-line" pathLength="100" stroke="var(--hb-ash)" strokeWidth="1.25" strokeLinecap="round"
...
export function AripanLoader({ label, className = '' }: { label: string; className?: string }) {
  return (
    <div className={`hb-loader ${className}`} role="status">
      <svg className="hb-loader-art" viewBox="0 0 160 44" fill="none" aria-hidden="true">
        <path className="hb-loader-line" pathLength="100" stroke="var(--hb-ash)" strokeWidth="1.5" strokeLinecap="round"
          d="M2 22 L100 22 C122 22 128 22 134 14 C140 6 152 8 155 17 C158 26 153 36 144 36 C137 36 133 30 136 25" />
      </svg>
      <span className="hb-loader-label">{label}</span>
    </div>
  )
}
...
export function MithilaBorder({ height = 22 }: { height?: number }) {
  ...
  const mid = height / 2
  return (
    <div className="hb-mithila-border" aria-hidden="true">
      {/* No viewBox: units are px, so the arches tile at a fixed size at any width. */}
      <svg className="hb-mithila-border-art" width="100%" height={height} preserveAspectRatio="none">
        <defs>
          <pattern id={pid} width="18" height={height} patternUnits="userSpaceOnUse">
            <path d={`M4 ${mid + 4} q0 -7 5 -7 t5 7`} fill="none" stroke="var(--hb-ruby)" strokeWidth="1.2" />
          </pattern>
        </defs>
        <rect width="100%" height={height} fill="var(--hb-pearl)" />
        <rect width="100%" height={height} fill={`url(#${pid})`} />
        <rect width="100%" height="1.6" y="3" fill="var(--hb-ruby)" />
        <rect width="100%" height="1.6" y={height - 4.6} fill="var(--hb-ruby)" />
      </svg>
    </div>
  )
}

...
import { NotchWing } from '@/components/brand/mithila'
...
export default function Navbar() {
  ...
  const searchField = (index: number, island = false) => (
    <form role="search" onSubmit={search} className={`nn-search ${island ? 'nn-search--island' : ''}`}>
      <label className="sr-only" htmlFor={`${menuId}-q${index}`}>Search a mohalla, a place, a town</label>
